...
export function showLoginRegisterWebview(
    ...
    <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>${mode === 'login' ? 'Login' : mode === 'register' ? 'Register' : 'Forgot Password'}</title>
        <link href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" rel="stylesheet">
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css">
        <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@300;400;500;600&display=swap" rel="stylesheet">
        <script src="https://cdnjs.cloudflare.com/ajax/libs/crypto-js/4.1.1/crypto-js.min.js"></script>
    ...
    <body>
        <div class="auth-form">
            <h2 class="text-center">${mode === 'login' ? 'Login' : mode === 'register' ? 'Register' : 'Forgot Password'}</h2>
            ${message_html}
            <form id="authForm">
                ${mode === 'login' ? `
                <div class="form-group">
            ...
            <div class="auth-link">
                <p>
                    ${mode === 'login' ? `
                        <a href="#" onclick="toggleMode('forgot')">Forgot Password?</a><br>
                        Don't have an account? <a href="#" onclick="toggleMode('register')">Register</a>`
                    : mode === 'register' ? `
                        Already have an account? <a href="#" onclick="toggleMode('login')">Login</a>`
                    : `
                        <a href="#" onclick="toggleMode('login')">Back to Login</a>`}
                </p>
            </div>
        </div>
        <script>
            ...
            function encryptData(requestData) {
                try {
                    const key = CryptoJS.enc.Utf8.parse("1234567890123456");
                    const iv = CryptoJS.enc.Utf8.parse("abcdefghijklmnop");
                    const encrypted = CryptoJS.AES.encrypt(requestData, key, { iv: iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 });
                    return encrypted.toString();
                } catch (error) {
                    vscode.postMessage({ command: 'log', message: "Encryption failed: " + error.toString() });
                    return { status: false, data: error.toString() };
            ...
            if (authForm) {
                authForm.addEventListener('submit', (event) => {
                    ...
                    if ('${mode}' === 'register') {
                        ...
                        fetch('https://dummyapi.io/reset-password', {
                            method: 'POST',
                        ...
                        .then(response => response.json())
                        .then(data => {
                            vscode.postMessage({ command: 'loginRegisterSuccess', message: 'Reset link sent to your email.' });
                        })
                        .catch(error => {
                            vscode.postMessage({ command: 'loginRegisterError', error: error.message || 'Reset failed.' });
                        });
