...
export class GenieCommandsProvider implements vscode.TreeDataProvider<GenieCommand | GenieCategory> {
  ...
  getChildren(element?: GenieCommand | GenieCategory): Thenable<(GenieCommand | GenieCategory)[]> {
    if (!element) {
      ...
      return Promise.resolve([
        new GenieCommand("Add Docstring", "extension.addDocstrings", 'output', 'Automatically add docstrings to your code.'),
      ...
      if (review_config === 'HSBC') {
        return Promise.resolve([
          ...
          new GenieCommand("Architecture Review", "extension.architectureReview", "terminal",  "Perform an architecture review")
        ]);
      }
      else {
        return Promise.resolve([
